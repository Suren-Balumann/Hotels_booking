...
async def get_hotel_by_id(hotel_id: int, db: DBDep):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Отеля не существует")
...
async def change_hotel_all_values(hotel_id: int, hotel_data: HotelAdd, db: DBDep):
    hotel = await db.hotels.get_filter_by(id=hotel_id)
    if len(hotel) < 1:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Not found")
    if len(hotel) > 1:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="More than one"
        )
